...
void TreeNode::updateMinimalTraverseList(const std::shared_ptr<Itemset>& crtItemset)
{
	...
	if (!only_minimal || minimalMt >= MAX_MINIMAL_TRAVERSE_SIZE || crtItemset->getItemCount() <= minimalMt)
	{
		...
		bool isEssential = ToExploreItemset::computeIsEssential(crtItemset, true);
		if (isEssential)
#endif
		{
			{
...
{	
	...
	// results of cumulated combined items / must be declared outside of the loop / this itemset is temporary
	ToExploreItemset cumulatedItemset;
	...
	for (unsigned int i = 0; i < toTraverse.size(); i++)
	{
		...
		else
		{
			if (i == 0)
			{
				...
				cumulatedItemset.combineItemset(crtItemset);
			}
			else
			{
				if(isCandidateForMaxClique(cumulatedItemset, crtItemset))
				{
					// add as max clique
					toExplore.push_back(crtItemset);
					// set cumulated temporary itemset
					cumulatedItemset.combineItemset(crtItemset);					
				}
				else
				{
#ifdef ISESSENTIAL_ON_TOEXPLORE
					bool isEssential = ToExploreItemset::computeIsEssential(crtItemset);
					if (isEssential)
#endif
					{
#ifdef TRACE
		...
		for (unsigned int i = toExplore_MaxClique_Index; i--; )
		{
			...
			try
			{
				...
				for_each(toExplore.begin(), toExplore.end(), [&newToTraverse, &toCombinedLeft](std::shared_ptr<Itemset>& toCombinedRight) {
					...
					{
						if(ToExploreItemset::isEssentialRapid(toCombinedLeft, toCombinedRight))
						{
