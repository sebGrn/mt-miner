...
Problem::~Problem()
{
  ...
  if (TB)       delete TB;
  if (S)        delete S;
  if (indexA)   delete indexA;
  if (indexB)   delete indexB;
  if (heapA)    delete heapA;
  if (heapB)    delete heapB;
  if (delA)     delete delA;
  if (delB)     delete delB;
}
...
// initialize stack other data structure before generation
{
  ...
  S->push(0,0,0,0,0,A->getNum()-1,0,B->getNum()-1,0,0,-1);
  computeEssentialsDown();
  computeEssentialsUp();
  if(debug==1){
	print();printf("\n");
...
//	  printf("Depth= %i Nodes=%i BNum=%i\n",S->num,nodesVisited,indexB->getNum());
    ...
    if(seq()==2){
	  ...
	  setSeq(seq()+1);             
	  S->push(0,0,0,0,Ab(),Ae(),Bb(),Be(),0,0,-1);
	  nodesVisited++;
